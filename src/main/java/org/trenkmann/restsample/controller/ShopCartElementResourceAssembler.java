...
public class ShopCartElementResourceAssembler implements ResourceAssembler<ShopCartElement, Resource<ShopCartElement>> {
  ...
  public Resource<ShopCartElement> toResource(ShopCartElement shopCartElement) {
      return new Resource<>( shopCartElement,
          linkTo(methodOn(ShopOrderController.class).getCartElementsByCartId(shopCartElement.getShopCart().getId())).withRel("elementsInCart"),
          linkTo(methodOn(MP3Controller.class).getMP3ById(shopCartElement.getMp3().getId())).withRel("mp3"),
          linkTo(methodOn(ShopOrderController.class).getCartElementByCartIdAndElementId(shopCartElement.getShopCart().getId(), shopCartElement.getId())).withSelfRel());
    }
